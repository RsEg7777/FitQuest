...
async function renderProgress() {
  ...
  // Pre-fetch stats summary counts
  const wCount   = await dbGet('SELECT COUNT(*) as c FROM workout_log WHERE user_id = ?', [userId]);
  const fCount   = await dbGet('SELECT COUNT(*) as c FROM food_log WHERE user_id = ?', [userId]);
  const burnRow  = await dbGet('SELECT COALESCE(SUM(calories_burned),0) as c FROM workout_log WHERE user_id = ?', [userId]);
  const chCount  = await dbGet('SELECT COUNT(*) as c FROM user_challenges WHERE user_id = ? AND completed = 1', [userId]);
  ...
  document.getElementById('page-content').innerHTML = `
    ...
    <div class="stats-grid">
      <div class="stat-card">
        ...
        <div class="stat-info">
          <div class="stat-value">${wCount ? wCount.c : 0}</div>
      ...
      <div class="stat-card">
        ...
        <div class="stat-info">
          <div class="stat-value">${fCount ? fCount.c : 0}</div>
      ...
      <div class="stat-card">
        ...
        <div class="stat-info">
          <div class="stat-value">${Math.round(burnRow ? burnRow.c : 0)}</div>
      ...
      <div class="stat-card">
        ...
        <div class="stat-info">
          <div class="stat-value">${chCount ? chCount.c : 0}</div>
  ...
  await drawCalorieChart(userId, profile);
  await drawHeatmap(userId);
}

async function drawWeightChart(userId) {
  const data = await dbAll('SELECT date, weight_kg FROM weight_log WHERE user_id = ? ORDER BY date ASC LIMIT 30', [userId]);
...
async function drawCalorieChart(userId, profile) {
  const days = [];
  for (let i = 6; i >= 0; i--) {
  ...
  const intake = [];
  for (const d of days) {
    const r = await dbGet('SELECT COALESCE(SUM(calories),0) as c FROM food_log WHERE user_id = ? AND date = ?', [userId, d]);
    intake.push(r ? r.c : 0);
  }
...
async function drawHeatmap(userId) {
  ...
  for (let w = 3; w >= 0; w--) {
    for (let d = 0; d < 7; d++) {
      ...
      const count = await dbGet('SELECT COUNT(*) as c FROM workout_log WHERE user_id = ? AND date = ?', [userId, dateStr]);
...
async function logWeight() {
  ...
  const userId = getCurrentUserId();
  await dbRun('INSERT INTO weight_log (user_id, date, weight_kg) VALUES (?, ?, ?)', [userId, getToday(), parseFloat(weight)]);
  await awardXP(userId, XP_REWARDS.weight_logged, 'weight_logged');
  await renderProgress();
}
